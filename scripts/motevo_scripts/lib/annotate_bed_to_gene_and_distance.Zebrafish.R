...
suppressPackageStartupMessages(library(scchicFuncs))
...
EntrezToSymbol.csv <- function(x, jsplit = ";"){
    ...
    xvec.symbol <- getSYMBOL(xvec, data = "org.Dr.eg.db")
...
regions <- subset(regions, seqnames %in% jchromos)  # remove X and Y?
...
if (!args$multi){
    ...
    regions.annotated <- as.data.frame(annotatePeak(regions.range, 
                                                    TxDb=TxDb.Drerio.UCSC.danRer11.knownGene, 
