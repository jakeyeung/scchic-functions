...
ReadMatAllele <- function(inf, as.sparse = TRUE){
  dat <- fread(inf)[-1, ] %>%
  ...
  dat$coord <- paste(paste(dat$chromo, paste(dat$start, dat$end, sep = "-"), sep = ":"), dat$allele, sep = ";")
  dat$chromo <- NULL
  # dat$startend <- NULL
  dat$start <- NULL
  dat$end <- NULL
...
mat <- ReadMatAllele(args$infile, as.sparse = TRUE)
