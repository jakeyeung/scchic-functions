...
m.louv <- ggplot(dat.umap.long, aes(x = umap1, y = umap2, color = cluster)) + geom_point() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
...
# dat.merge$cluster <- paste("cluster", dat.merge$cluster, sep = "")
